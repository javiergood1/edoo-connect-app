...
import { User, Mail, Calendar, Briefcase, ArrowRight } from 'lucide-react';
import useWizardStore from '../../../stores/wizardStore';

const Step1PersonalInfoFinal = () => {
  const { formData, setStepData, nextStep } = useWizardStore();
  const stepData = formData.step1 || {};

  // Estado local para evitar re-renders - SIN autoguardado automático
  ...
  // Manejar cambios en campos de texto - SIN guardado automático
  ...
  // Manejar cambios en sliders - SIN guardado automático
  const handleSliderChange = useCallback((sliderName) => (value) => {
    setLocalData(prev => ({ ...prev, [sliderName]: value[0] }));
  }, []);
...
export default Step1PersonalInfoFinal;
